...
3. **Planning documents are scattered across the root.**
   `ISSUE_FIX_PLAN.md`, `PREMIUM_MOBILE_APP_UI_PLAN.md`, `saved-setups-tab-update-plan.md`, `ict-framework-handover.md`, and this file are all root-level, while other plans live in `docs/archive/superseded-design/ui-redesign/` and `docs/archive/completed-fix-lists/`. Consolidate under `docs/plans/` so the root contains only app files, config, README, and CHANGELOG.
...
10. **Misc hygiene.**
    ...
    - `docs/archive/completed-fix-lists/2026-07-07-am-session.md` has spaces in the filename; use kebab-case (`2026-07-07-am-session.md`) and date-prefix fix-lists.
...
| 1.4 | Rename spaced filenames; add `LICENSE` decision | `docs/archive/completed-fix-lists/` |
